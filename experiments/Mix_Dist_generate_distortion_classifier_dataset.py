...
class QualityConverter():
  ...
  def mixedDistortion(self, blur_lvl_list, noise_lvl_list, end=10000):
    ...
    for i, (img, label) in enumerate(self.dataLoader):
      ...
      noise_lvl = noise_lvl_list[np.random.choice(len(noise_lvl_list), 1)[0]]
      ...
      distorted_img = self.__applyGaussianBlur(img, blur_lvl)
      distorted_img = transforms.ToTensor()(distorted_img).unsqueeze(0)
      distorted_img = self.__applyGaussianNoise(distorted_img, noise_lvl)
...
if __name__ == "__main__":
  ...
  parser.add_argument('--distortion_type', type=str, default="pristine", 
    choices=['pristine', 'gaussian_blur', 'gaussian_noise', 'mixed_distortion'],
    help='Distortion Type (default: pristine)')
  ...
  elif (args.distortion_type == "mixed_distortion"):
    quality_converter.mixedDistortion(blur_lvl_list, noise_lvl_list)
