...
def select_predictions(conf_list, class_list, p_tar):
...
def evaluate_model(model, loader, device, p_tar):
    ...
    with torch.no_grad():
        for data, target in tqdm(loader, leave=False):
            ...
            pred = select_predictions(conf_list, class_list, p_tar)
...
def evaluate_all():
    ...
    for blur_level in blur_levels:
        for noise_level in noise_levels:
            ...
            for model_name, model in models.items():
                ...
                acc = evaluate_model(model, loader, device, p_tar)
    ...
    df.to_csv(
      os.path.join("experiments", "figure_3_style_results.csv"),
      index=False
    )
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join("experiments", "figure_3_style.png"),
        dpi=300
    )
    plt.show()
